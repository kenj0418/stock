...
const getLatestPrice = async securityId => {
  const prices = await PricingHistory.find({ securityId });
...
const SecurityType = new GraphQLObjectType({
  ...
  fields: () => ({
    ...
    latestPrice: {
      ...
      resolve(parent, args) {
        return getLatestPrice(parent.id);
      }
    }
  })
});
...
const getCurrentValue = async (securityId, shares) => {
  const price = await getLatestPrice(securityId);
